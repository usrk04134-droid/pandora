...
from message_names import MessageName
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--plc-ip",
        action="store",
        default=os.getenv("TESTZILLA_PLC_IP", "192.168.100.10"),
        help="PLC IP address (env: TESTZILLA_PLC_IP)",
    )
    parser.addoption(
        "--adaptio-ip",
        action="store",
        default=os.getenv("TESTZILLA_ADAPTIO_IP", "192.168.101.151"),
        help="Adaptio external IP address (env: TESTZILLA_ADAPTIO_IP)",
    )
    parser.addoption(
        "--adaptio-username",
        action="store",
        default=os.getenv("TESTZILLA_ADAPTIO_USERNAME", "esab"),
        help="Adaptio SSH username (env: TESTZILLA_ADAPTIO_USERNAME)",
    )
    parser.addoption(
        "--adaptio-password",
        action="store",
        default=os.getenv("TESTZILLA_ADAPTIO_PASSWORD", "esab"),
        help="Adaptio SSH password (env: TESTZILLA_ADAPTIO_PASSWORD)",
    )
    parser.addoption(
        "--web-hmi-port",
        action="store",
        default=os.getenv("TESTZILLA_WEB_HMI_PORT", "8080"),
        help="WebHMI WebSocket port (env: TESTZILLA_WEB_HMI_PORT)",
    )
    parser.addoption(
        "--power-supply-ip",
        action="store",
        default=os.getenv("TESTZILLA_POWER_SUPPLY_IP", ""),
        help="Bench power supply IP address (env: TESTZILLA_POWER_SUPPLY_IP)",
    )
    parser.addoption(
        "--power-supply-port",
        action="store",
        type=int,
        default=int(os.getenv("TESTZILLA_POWER_SUPPLY_PORT", "0")),
        help="Bench power supply TCP port, 0 = auto-detect (env: TESTZILLA_POWER_SUPPLY_PORT)",
    )
    parser.addoption(
        "--power-supply-model",
        action="store",
        default=os.getenv("TESTZILLA_POWER_SUPPLY_MODEL", ""),
        help="Bench power supply model (keysight_e36234a | aimtti_cpx200dp), empty = auto-detect (env: TESTZILLA_POWER_SUPPLY_MODEL)",
    )
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.PLC_IP = config.getoption("--plc-ip")
    config.PLC_JSON_RPC_URL = f"https://{config.PLC_IP}/api/jsonrpc"
    config.ADAPTIO_EXTERNAL_IP = config.getoption("--adaptio-ip")
    config.ADAPTIO_RESTART_WAIT_TIME = 5  # seconds
    config.ADAPTIO_USERNAME = config.getoption("--adaptio-username")
    config.ADAPTIO_PASSWORD = config.getoption("--adaptio-password")
    ...
    web_hmi_port = config.getoption("--web-hmi-port")
    config.WEB_HMI_URI = f"ws://{config.ADAPTIO_EXTERNAL_IP}:{web_hmi_port}"

    # Bench power supply
    config.POWER_SUPPLY_IP = config.getoption("--power-supply-ip")
    config.POWER_SUPPLY_PORT = config.getoption("--power-supply-port")
    config.POWER_SUPPLY_MODEL = config.getoption("--power-supply-model")
    ...
    config.addinivalue_line(
        "markers", "skip_log_setup_teardown: skip Adaptio log cleanup/setup and collection/teardown fixtures"
    )

    config.addinivalue_line(
        "markers", "gen2: mark test as weld operation test (WPP, WDS, arc state)"
    )
    config.addinivalue_line(
        "markers", "plc_powersource: mark test as PLC power-source test using bench PSU"
    )
    ...
    def configure_settings(self, settings: dict[str, Any]) -> bool:
        ...
        try:
            response = self.web_hmi.send_and_receive_message(
                condition=None,
                request_name=MessageName.SET_SETTINGS.value,
                response_name=MessageName.SET_SETTINGS_RSP.value,
                payload=settings,
            )
...
def delete_adaptio_logs_fixture(request: pytest.FixtureRequest) -> None:
    ...
    if request.node.get_closest_marker("skip_log_setup_teardown"):
        logger.debug("Skipping Adaptio log setup for this test (skip_log_setup_teardown marker)")
        return
...
def collect_adaptio_logs_fixture(request: pytest.FixtureRequest) -> Iterator[None]:
    ...
    if request.node.get_closest_marker("skip_log_setup_teardown"):
        logger.debug("Skipping Adaptio log teardown for this test (skip_log_setup_teardown marker)")
        yield
        return
...
def set_joint_geometry(
    ...
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
            request_name=MessageName.SET_JOINT_GEOMETRY.value,
            response_name=MessageName.SET_JOINT_GEOMETRY_RSP.value,
            payload={
...
def delete_adaptio_database(adaptio_manager: "AdaptioManager", db_path) -> bool:
    """Stop Adaptio, delete the database file, and restart. Returns True on success."""
    try:
        adaptio_manager.stop_adaptio()
        adaptio_manager.manager.execute_command(command=f"rm -f {db_path}", sudo=True)
        logger.info(f"Deleted Adaptio database at {db_path}")
        adaptio_manager.start_adaptio()
        return True
    except Exception:
        logger.exception("Failed to delete and recreate database")
        return False
...
def add_weld_process_parameters(web_hmi: AdaptioWebHmi, **kwargs) -> bool:
    try:
        ...
        result = getattr(response, "result", None) or response.payload.get("result")
        if response and result == "ok":
            logger.info(f"Added weld process parameters: {kwargs.get('name', '')}")
            return True
        else:
...
def add_weld_data_set(web_hmi: AdaptioWebHmi, name: str, ws1_wpp_id: int, ws2_wpp_id: int) -> bool:
    try:
        ...
        result = getattr(response, "result", None) or response.payload.get("result")
        if response and result == "ok":
            logger.info(f"Added weld data set: {name}")
            return True
        else:
...
def select_weld_data_set(web_hmi: AdaptioWebHmi, weld_data_set_id: int) -> bool:
    try:
        ...
        result = getattr(response, "result", None) or response.payload.get("result")
        if response and result == "ok":
            logger.info(f"Selected weld data set: {weld_data_set_id}")
            return True
        else:
...
def get_weld_process_parameters(web_hmi: AdaptioWebHmi) -> list | None:
    """Get weld process parameters from Adaptio."""
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
        ...
        if response and response.payload:
            if isinstance(response.payload, list):
                ...
                for key in ['items', 'parameters', 'weldProcessParameters']:
                    if key in response.payload and isinstance(response.payload[key], list):
                        return response.payload[key]
                logger.warning(f"Unexpected payload structure: {response.payload}")
                return []
        return None
    except Exception:
...
def get_weld_data_sets(web_hmi: AdaptioWebHmi) -> list | None:
    """Get weld data sets from Adaptio."""
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
        ...
        logger.debug(f"Received GetWeldDataSets response: {response}")
        
        if response and response.payload:
            if isinstance(response.payload, list):
                return response.payload
            elif isinstance(response.payload, dict):
                for key in ['items', 'dataSets', 'weldDataSets']:
                    if key in response.payload and isinstance(response.payload[key], list):
                        return response.payload[key]
                logger.warning(f"Unexpected payload structure: {response.payload}")
                return []
...
def get_weld_programs(web_hmi: AdaptioWebHmi) -> list | None:
    """Get weld programs from Adaptio."""
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
        ...
        if response and response.payload:
            if isinstance(response.payload, list):
                ...
                for key in ['items', 'programs', 'weldPrograms']:
                    if key in response.payload and isinstance(response.payload[key], list):
                        return response.payload[key]
                logger.warning(f"Unexpected payload structure: {response.payload}")
                return []
...
def remove_weld_data_set(web_hmi: AdaptioWebHmi, wds_id: int) -> bool:
    ...
    try:
        ...
        result = getattr(response, "result", None) or response.payload.get("result") if isinstance(response.payload, dict) else None
        if response and result == "ok":
            logger.info(f"Removed weld data set: {wds_id}")
            return True
        else:
...
# ---------------------------------------------------------------------------
# Bench power supply fixture
# ---------------------------------------------------------------------------

@pytest.fixture(name="bench_power_supply", scope="module")
def bench_power_supply_fixture(request: pytest.FixtureRequest):
    ...
    ip = request.config.POWER_SUPPLY_IP
    if not ip:
        pytest.skip("Bench power supply not configured (--power-supply-ip missing)")

    conn = {"ip_address": ip}
    port = request.config.POWER_SUPPLY_PORT
    if port:
        conn["port"] = port

    model = request.config.POWER_SUPPLY_MODEL
    if model:
        psu = PowerSupplyFactory.create(model, conn)
    else:
        psu = PowerSupplyFactory.auto_detect(conn)
...
def set_abw_simulation(
    ...
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
            request_name=MessageName.ABW_SIM_CONFIG_SET.value,
            response_name=MessageName.ABW_SIM_CONFIG_SET_RSP.value,
            payload={
...
def start_abw_simulation(
    ...
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
            request_name=MessageName.ABW_SIM_START.value,
            response_name=MessageName.ABW_SIM_START_RSP.value,
...
def stop_abw_simulation(web_hmi: AdaptioWebHmi) -> bool:
    ...
    try:
        response = web_hmi.send_and_receive_message(
            condition=None,
            request_name=MessageName.ABW_SIM_STOP.value,
            response_name=MessageName.ABW_SIM_STOP_RSP.value,
            payload={}
        )
        logger.debug(f"Received response: {response}")
        result = getattr(response, "result", None) or response.payload.get("result")
...
def joint_tracking_sim_setup_fixture(request: pytest.FixtureRequest, addresses, setup_plc) -> None:
    ...
    try:
        ...
        try:
            response = web_hmi.send_and_receive_message(
                condition=None,
                request_name=MessageName.LASER_TORCH_CAL_SET.value,
                response_name=MessageName.LASER_TORCH_CAL_SET_RSP.value,
        ...
        try:
            response = web_hmi.send_and_receive_message(
                condition=None,
                request_name=MessageName.WELD_OBJECT_CAL_SET.value,
                response_name=MessageName.WELD_OBJECT_CAL_SET_RSP.value,
                payload=    {"residualStandardError" :  0.00,
                            ...
                            "wireDiameter": 11.0, "stickout": 25.0}
            )
            logger.debug(f"Received response: {response}")
            result = getattr(response, "result", None) or response.payload.get("result")
            if response and result == "ok":
